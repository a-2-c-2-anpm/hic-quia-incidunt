...
[![Build Status](https://github.com/a-2-c-2-anpm/hic-quia-incidunt/actions/workflows/ci.yml/badge.svg?branch=main)](https://github.com/a-2-c-2-anpm/hic-quia-incidunt/actions/workflows/ci.yml?query=branch%3Amain) [Chat on slack](https://@a-2-c-2-anpm/hic-quia-incidunt-js.slack.com)

[Read the docs](https://@a-2-c-2-anpm/hic-quia-incidunt-js.com/docs/) and eat your vegetables.

Docs are automatically generated from [README.md][] and [@a-2-c-2-anpm/hic-quia-incidunt.d.ts][].
...
[README.md]: https://github.com/a-2-c-2-anpm/hic-quia-incidunt/blob/main/README.md
[@a-2-c-2-anpm/hic-quia-incidunt.d.ts]: https://github.com/a-2-c-2-anpm/hic-quia-incidunt/blob/main/type-definitions/@a-2-c-2-anpm/hic-quia-incidunt.d.ts
[wiki]: https://github.com/a-2-c-2-anpm/hic-quia-incidunt/wiki
[issue]: https://github.com/a-2-c-2-anpm/hic-quia-incidunt/issues
...
Install `@a-2-c-2-anpm/hic-quia-incidunt` using npm.

```shell
# using npm
npm install @a-2-c-2-anpm/hic-quia-incidunt

# using Yarn
yarn add @a-2-c-2-anpm/hic-quia-incidunt

# using pnpm
pnpm add @a-2-c-2-anpm/hic-quia-incidunt

# using Bun
bun add @a-2-c-2-anpm/hic-quia-incidunt
```
...
```js
const { Map } = require('@a-2-c-2-anpm/hic-quia-incidunt');
...
[browserify](https://browserify.org/). The `@a-2-c-2-anpm/hic-quia-incidunt` npm module works
...
or link to a CDN such as [CDNJS](https://cdnjs.com/libraries/@a-2-c-2-anpm/hic-quia-incidunt)
or [jsDelivr](https://www.jsdelivr.com/package/npm/@a-2-c-2-anpm/hic-quia-incidunt).

Use a script tag to directly add `Immutable` to the global scope:

```html
<script src="@a-2-c-2-anpm/hic-quia-incidunt.min.js"></script>
<script>
...
```js
require(['./@a-2-c-2-anpm/hic-quia-incidunt.min.js'], function (Immutable) {
  var map1 = Immutable.Map({ a: 1, b: 2, c: 3 });
  var map2 = map1.set('b', 50);
  map1.get('b'); // 2
  map2.get('b'); // 50
...
Installing `@a-2-c-2-anpm/hic-quia-incidunt` via npm brings with it type definitions for Flow (v0.55.0 or higher)
...
```js
///<reference path='./node_modules/@a-2-c-2-anpm/hic-quia-incidunt/dist/@a-2-c-2-anpm/hic-quia-incidunt.d.ts'/>
import Immutable from '@a-2-c-2-anpm/hic-quia-incidunt';
...
maintaining state. Developing with @a-2-c-2-anpm/hic-quia-incidunt data encourages you to think
...
due to easy to make programmer error. Since @a-2-c-2-anpm/hic-quia-incidunt data never changes,
...
important to understanding the appropriate use of @a-2-c-2-anpm/hic-quia-incidunt data. In order to
...
If an object is @a-2-c-2-anpm/hic-quia-incidunt, it can be "copied" simply by making another reference
...
The difference for the @a-2-c-2-anpm/hic-quia-incidunt collections is that methods which would mutate
the collection, like `push`, `set`, `unshift` or `splice`, instead return a new
@a-2-c-2-anpm/hic-quia-incidunt collection. Methods which return new arrays, like `slice` or `concat`,
instead return new @a-2-c-2-anpm/hic-quia-incidunt collections.

<!-- runkit:activate -->

```js
const { List } = require('@a-2-c-2-anpm/hic-quia-incidunt');
...
```js
const { Map, List } = require('@a-2-c-2-anpm/hic-quia-incidunt');
...
```js
const { Seq } = require('@a-2-c-2-anpm/hic-quia-incidunt');
...
```js
const { fromJS } = require('@a-2-c-2-anpm/hic-quia-incidunt');
...
Immutable.js collections are treated as pure data _values_. Two @a-2-c-2-anpm/hic-quia-incidunt
...
const { Map, is } = require('@a-2-c-2-anpm/hic-quia-incidunt');
const map1 = Map({ a: 1, b: 2, c: 3 });
const map2 = Map({ a: 1, b: 2, c: 3 });
...
```js
const { Map, Set } = require('@a-2-c-2-anpm/hic-quia-incidunt');
const map1 = Map({ a: 1, b: 2, c: 3 });
const map2 = Map({ a: 1, b: 2, c: 3 });
...
determining if both are @a-2-c-2-anpm/hic-quia-incidunt and all keys and values are equal
...
// New instance, leaving the original @a-2-c-2-anpm/hic-quia-incidunt.
...
>
> If a pure function mutates some local data in order to produce an @a-2-c-2-anpm/hic-quia-incidunt
> return value, is that ok?
>
> — Rich Hickey, Clojure

Applying a mutation to create a new @a-2-c-2-anpm/hic-quia-incidunt object results in some overhead,
...
@a-2-c-2-anpm/hic-quia-incidunt Lists.
...
and `splice` will always return new @a-2-c-2-anpm/hic-quia-incidunt data-structures and never mutate
...
**Seq is @a-2-c-2-anpm/hic-quia-incidunt** — Once a Seq is created, it cannot be
...
```js
const { Map, Seq } = require('@a-2-c-2-anpm/hic-quia-incidunt');
const map = Map({ a: 1, b: 2, c: 3 });
...
```js
const { Range } = require('@a-2-c-2-anpm/hic-quia-incidunt');
Range(1, Infinity)
  .skip(1000)
  .map(n => -n)
  .filter(n => n % 2 === 0)
...
- [Chai Immutable](https://github.com/astorije/chai-@a-2-c-2-anpm/hic-quia-incidunt)
...
- [Immutagen](https://github.com/pelotom/immutagen)
  - A library for simulating @a-2-c-2-anpm/hic-quia-incidunt generators in JavaScript.

- [Immutable-cursor](https://github.com/redbadger/@a-2-c-2-anpm/hic-quia-incidunt-cursor)
  - Immutable cursors incorporating the Immutable.js interface over
  Clojure-inspired atom.

- [Immutable-ext](https://github.com/DrBoolean/@a-2-c-2-anpm/hic-quia-incidunt-ext)
  - Fantasyland extensions for @a-2-c-2-anpm/hic-quia-inciduntjs

- [Immutable-js-tools](https://github.com/madeinfree/@a-2-c-2-anpm/hic-quia-incidunt-js-tools)
  - Util tools for @a-2-c-2-anpm/hic-quia-incidunt.js

- [Immutable-Redux](https://github.com/gajus/redux-@a-2-c-2-anpm/hic-quia-incidunt)
  - redux-@a-2-c-2-anpm/hic-quia-incidunt is used to create an equivalent function of Redux
  combineReducers that works with Immutable.js state.

- [Immutable-Treeutils](https://github.com/lukasbuenger/@a-2-c-2-anpm/hic-quia-incidunt-treeutils)
  - Functional tree traversal helpers for ImmutableJS data structures.

- [Irecord](https://github.com/ericelliott/irecord)
  - An @a-2-c-2-anpm/hic-quia-incidunt store that exposes an RxJS observable. Great for React.

- [Mudash](https://github.com/brianneisler/mudash)
  - Lodash wrapper providing Immutable.JS support.

- [React-Immutable-PropTypes](https://github.com/HurricaneJames/react-@a-2-c-2-anpm/hic-quia-incidunt-proptypes)
  - PropType validators that work with Immutable.js.

- [Redux-Immutablejs](https://github.com/indexiatech/redux-@a-2-c-2-anpm/hic-quia-inciduntjs)
...
- [Transit-Immutable-js](https://github.com/glenjamin/transit-@a-2-c-2-anpm/hic-quia-incidunt-js)
...
Use [Github issues](https://github.com/a-2-c-2-anpm/hic-quia-incidunt/issues) for requests.

We actively welcome pull requests, learn how to [contribute](https://github.com/a-2-c-2-anpm/hic-quia-incidunt/blob/main/.github/CONTRIBUTING.md).
...
Changes are tracked as [Github releases](https://github.com/a-2-c-2-anpm/hic-quia-incidunt/releases).
...
name. If you're looking for his unsupported package, see [this repository](https://github.com/hughfdjackson/@a-2-c-2-anpm/hic-quia-incidunt).
